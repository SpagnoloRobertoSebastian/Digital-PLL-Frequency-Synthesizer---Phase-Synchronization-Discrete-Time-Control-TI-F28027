...
//Con está versión se consiguio error nulo a
// la rampa
...
disp('Pm_k= 0.0618551*P_k_1+ 0.9690724*Pm_k_1');
        a1=0.0579437;
        a2=0.9420563;
       Pm_k=a1*P_k_1+a2*Pm_k_1   //me llama la atención que no dependa de la actual
      //Pm_k= 0.0618551*P_k+ 0.9690724*Pm_k_1// filtro del informe
...
figure(2);clf;
 subplot(2,1,1)
 title('Producto - salida Multiplicador ')
 plot(t1(1,1:N*kc), Producto)
 xgrid(color("gray")), xlabel("[seg]")
 subplot(2,1,2)
 title('Pmedio- salida del filtro')
 plot(t1(1,1:N*kc), Pmedio,'r')
 xgrid(color("gray")), xlabel("[seg]")
